...
public class NativeUtils {
   ...
   * into a temporary directory and then loaded into memory, and will be deleted after Java runtime
   * exits.
   */
    ...
    try (InputStream inputStream = NativeUtils.class.getResourceAsStream(pathInJar)) {
      if (inputStream == null) {
        ...
        throw new FileNotFoundException(
            String.format("The WeDPR dynamic library file was not found in '%s'.", pathInJar));
      }
  ...
  private static String checkAndExtractLibFilename(String pathInJar) {
    ...
    if (filename == null || filename.length() < MIN_FILENAME_LENGTH) {
      throw new IllegalArgumentException(
          String.format("The filename '%s' seems too short. Please check again", filename));
    }
    return filename;
  }
